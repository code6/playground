...
public:
    ...
    void parse_location(vector<string>& loc_data) {
        int loc_num = loc_data.size();
        const char* cols[9];

        for (int i = 0; i < loc_num; i++) {
            parse_line(loc_data[i].c_str(), cols, 9);
            int loc    = atoi(cols[0]);
            int lati   = atoi(cols[7]); 
            ...
            vector <string> experimentData) 
    {
        ...
        vector<float> result1  = LearnMF(train, test);
        ...
        vector<float> result5 = LearnMF(train, test);
                ...
                vector<double> result(result1.size());

        for (int i = 0; i < (int)result.size(); i++)
        {
            result[i] = 0.2*(result1[i] + result2[i] + result3[i] + result4[i] + result5[i]);
        }    
        return result;
    }
};
