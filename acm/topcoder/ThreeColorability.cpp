...
bool test() {
    ...
    while (!Q.empty()) {
        PII cur = Q.front();
        Q.pop();
        range(i, d[cur.fs][cur.sc].size()) {
            PII cand = d[cur.fs][cur.sc][i];
            if (color[cand.fs][cand.sc] == -1) {
                int mask = 0;
                range(k, d[cand.fs][cand.sc].size()) {
                    PII next = d[cand.fs][cand.sc][k];
                ...
                if (mask == 3 || mask == 5 || mask == 6) {
                    color[cand.fs][cand.sc] = get_val(7 - mask);
                    Q.push(cand);
                }
            }
        }
